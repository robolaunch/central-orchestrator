 ...
 * from a library
 * called <a href="https://github.com/secure-io/sio-go">sio-go</a>. The library
 * encrypts/decrypts
 * data in chunks, which allows it handle large amounts of data, without
 * sacrificing security. In
 * addition, MinIO itself formats the data into specific format, to allow
...
public class Crypto {
   ...
   * The format returned is compatible with MinIO servers and clients. Header
   * format: salt [string
   * 32] | aead id [byte 1] | nonce [byte 8] | encrypted_data [byte
   * len(encrypted_data)] To see the
   * original implementation in Go, check out the <a
   * href="https://github.com/minio/madmin-go/blob/main/encrypt.go#L38">madmin-go
   * library</a>.
   *
  ...
  public static byte[] encrypt(String password, byte[] data)
    ...
    /**
     * NONCE is expected to be 12-bytes for AES-GCM. We add 4 empty bytes, which we
     * increment in
    ...
    /**
     * Increment IV (nonce) by 1 as we used it for generating a tag for additional
     * data.
     */
    paddedNonce[8] = 1;
  ...
  public static byte[] decrypt(String password, byte[] payload)
    ...
    /**
     * One byte to determine which encryption format to use. We only allow for
     * Argon2ID AES-GCM.
     */
    payloadBuffer.get();
    ...
    /**
     * Nonce for AES-GCM is expected to be 12 bytes, but we keep it at 8-bytes to
     * allow up to
