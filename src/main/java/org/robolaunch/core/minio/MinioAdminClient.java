...
package org.robolaunch.core.minio;
...
import org.robolaunch.core.minio.messages.DataUsageInfo;
...
public class MinioAdminClient {
  ...
  private Response execute(
    ...
    HttpUrl.Builder urlBuilder = this.baseUrl
        .newBuilder()
    ...
    if (traceStream != null) {
      String trace = response.protocol().toString().toUpperCase(Locale.US)
          + " "
          + response.code()
          + "\n"
          + response.headers();
   ...
   * @throws NoSuchAlgorithmException   thrown to indicate missing of MD5 or
   *                                    SHA-256 digest library.
   * @throws InvalidKeyException        thrown to indicate missing of HMAC SHA-256
   *                                    library.
   * @throws IOException                thrown to indicate I/O error on MinIO REST
   *                                    operation.
   * @throws InvalidCipherTextException thrown to indicate data cannot be
   *                                    encrypted/decrypted.
   */
  public void addUser(
   ...
   * @throws NoSuchAlgorithmException thrown to indicate missing of MD5 or SHA-256
   *                                  digest library.
   * @throws InvalidKeyException      thrown to indicate missing of HMAC SHA-256
   *                                  library.
   * @throws IOException              thrown to indicate I/O error on MinIO REST
  ...
  public void addUpdateGroup(
    ...
    GroupAddUpdateRemoveInfo groupAddUpdateRemoveInfo = new GroupAddUpdateRemoveInfo(group, groupStatus, members,
        false);
  ...
  public GroupInfo getGroupInfo(String group)
      throws NoSuchAlgorithmException, InvalidKeyException, IOException {
    try (Response response = execute(Method.GET, Command.GROUP_INFO, ImmutableMultimap.of("group", group), null)) {
      byte[] jsonData = response.body().bytes();
